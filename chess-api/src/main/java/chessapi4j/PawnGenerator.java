...
final class PawnGenerator {
	...
	public PawnGenerator(VisibleMetrics visibleMetrics, GeneratorUtil generatorUtil, CheckMetrics checkMetrics) {
	...
	public long pawnMoves(long br, int square, int[] pawnsDirections, int pieceType, int[][] matrix1, int[][] matrix2,
			int kingSquare, long enemies, long friends, int ep, long wm, long[] bitboards, long checkMask, long inCheckMask,
			long nextWhiteMove) {
		final int[] captureArray = matrix2[square];
		long captureMoves = 0L;
		long captureCoronationMoves = 0L;
		...
		final long legalCoronationMoves = pseudoCoronationMoves & pinMask2 & inCheckMask;
		final long legalAdvanceEnPassantMoves = advanceEnPassantMoves & pinMask3 & inCheckMask;
		return legalMoves | legalCoronationMoves | legalAdvanceEnPassantMoves | generateEnPassantCapturesBitboard(
				possibleEnPassant, pieceType, square, pawnsDirections, captureArray, bitboards, wm);
	}
	...
	private long generateEnPassantCapturesBitboard(long moves, int pieceType, int originSquare, int[] pawnsDirections,
			int[] captureArray, long[] bitboards, long wm) {
		var movesCopy = moves;
		var legalEpCaptures = 0L;
		while (movesCopy != 0L) {
			final long move = movesCopy & -movesCopy;
			final long capture = 1L << (generatorUtil.squaresMap(move) + EP_CHOICE[(int) wm]);
			var newBitboards = new long[12];
			System.arraycopy(bitboards, 0, newBitboards, 0, 12);
			...
			if (checkMetrics.isInCheck(newBitboards, wm == 1L))
				legalEpCaptures |= move;
			movesCopy = movesCopy & ~move;
		}
		return legalEpCaptures;
	}
